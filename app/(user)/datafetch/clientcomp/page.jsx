...
const FetchClientComp = () => {
    const [userInfo, setUserInfo] = useState({});
    const searchParams = useSearchParams();
    const userName = searchParams.get("name") || "john";
    ...
    const percent = (userInfo.probability || 0) * 100
        ...
        <div className='min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-100 flex items-center justify-center p-4'>
            <div className={`bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full ${isMale ? 'border-l-4 border-blue-500' : 'border-l-4 border-pink-500'}`}>
                <div className="text-center flex items-center justify-center flex-col">
                    ...
                    {userInfo.gender ? (<p className={`${isMale ? 'text-blue-500 bg-blue-200' : 'text-pink-500 bg-pink-200'} text-lg font-semibold mb-2 rounded-full w-fit px-4 py-0.5 flex items-center gap-1`}>
                        {isMale ? <Mars className='text-blue-500 w-5 h-5' /> : <Venus className='text-pink-500 w-5 h-5' />} {userInfo.gender.charAt(0).toUpperCase() + userInfo.gender.slice(1)}
                    </p>) : (
...
export default FetchClientComp;